...
void sendToThingSpeakMQTT() {
    ...
    if (tempC < 33.0) statusMessage += "Low Temp ";
    else if (tempC > 38.0) statusMessage += "High Temp ";

    if (spo2 < 94.0) statusMessage += "Low SpO2 ";

    if (bpm < 50) statusMessage += "Low BPM ";
    else if (bpm > 120) statusMessage += "High BPM ";

    if (magnitude > 2.8 || magnitude < 2.0) statusMessage += "Fall Detected ";

    if (statusMessage == "") statusMessage = "All Normal";

    String payload = "field1=" + String(tempC, 1) +
                   "&field2=" + String(spo2, 1) +
                   "&field3=" + String(hr, 1) +
                   "&field4=" + String(bpm) +
                   "&field5=" + String(magnitude, 2) +
                   "&field6=" + String(pad) + 
...
void runCode1(unsigned long currentMillis) {
  if (currentMillis - previousCycle >= 2000) {
    ...
    switch (phaseCode1) {
      case 0: {
        tempC = (analogRead(TEMP_PIN) * (3.3 / 4095.0) * 100);
        tempC = 0.2747 * tempC + 25.8756;
      ...
      case 1: {
        ...
        hr = -0.000306*redValue+0.001224*irValue+57.09;  // Placeholder formula
        spo2 = -0.0636*(redValue/1000)-(0.1621*(irValue/1000))+113.30; // Placeholder formula
  ...
  if (currentMillis - lastUpdate >= 200) {
    ...
    switch (phase) {
      ...
      case 2: {
        ...
        magnitude = sqrt(xg * xg + yg * yg + zg * zg);
...
bool checkTouch() {
  if (digitalRead(TOUCH_1)) { lcd.setCursor(0,1); lcd.print("YES"); pad = 1; return true; }
  if (digitalRead(TOUCH_2)) { lcd.setCursor(0,1); lcd.print("NO");  pad = 2; return true; }
  if (digitalRead(TOUCH_3)) { lcd.setCursor(0,1); lcd.print("Netural"); pad=3; return true; }
  if (digitalRead(TOUCH_4)) { lcd.setCursor(0,1); lcd.print("Help"); pad=4; return true;}
  lcd.setCursor(0,1); lcd.print("No Touch"); pad=0; return false;
}
